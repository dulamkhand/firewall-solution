...
<?php foreach ($pager->getResults() as $image): ?>
  <div style="position:relative;width:180px;height:150px;overflow:hidden;padding:1px;float:left;margin:4px;border:1px solid #ccc;">
    ...
    <a style="position:absolute;bottom:0px;right:14px;" href="<?php echo url_for('gallery/edit?id='.$image->getId()) ?>"><?php echo image_tag('i/edit.gif', array())?></a>
    <a style="position:absolute;bottom:0px;right:0px;" onclick="return confirm('Та итгэлтэй байна уу?')" href="<?php echo url_for('gallery/delete?id='.$image->getId()) ?>"><?php echo image_tag('i/delete.gif', array())?></a>
  </div>
<?php endforeach; ?>
